...
Value evaluate(const Position& pos) {
    ...
    // --- material + PSQT + mobility + zone effect, single pass over pieces ---
    Bitboard b = occ;
    while (b.any()) {
        Square s = b.pop_lsb();
    ...
    auto king_penalty = [](int atk, int def) {
        int danger = atk * 12 - def * 6;
        if (danger < 0) danger = 0;
        int penalty = danger + danger * danger / 64;
        return penalty > 900 ? 900 : penalty;
    };
    if (haveBK) score -= king_penalty(bAtk, bDef);
    if (haveWK) score += king_penalty(wAtk, wDef);
